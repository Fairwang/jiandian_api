...
def getmsg():
    ...
    sql ='SELECT *FROM t_msg_sms WHERE mobile="15868314566" ORDER BY id DESC'
    # 建立连接，使用MMySQLdb模块的connect方法连接mysql，传入账号、密码、数据库、端口、ip和字符集
    coon = MySQLdb.connect(user='root', passwd='fr349fg97^*&#TY', db='jdshop_test', port=3306, host='116.62.112.180', charset='utf8')
    cursor = coon.cursor()
    try:
        cursor.execute(sql)
...
def interfaceTest(case_list, file_path):
    ...
    for case in case_list:
        ...
        login_token_json=json.loads(results)
        content=login_token_json.get("content")
        print  "zheshi x:%s" %content
        #login_token 解密
        for i in content:
            if i=='login_token':
                login_token_encode=(content['login_token'])
                private_key_str = file('rsa_private.pem', 'rb').read()
                private_key = RSA.load_key_string(private_key_str)
                login_token = base64.decodestring(login_token_encode)
                de_data = private_key.private_decrypt(login_token, RSA.pkcs1_padding)
                print "zheshi de_data %s" %de_data
                login_token_str="{"+'"'+"login_token"+'"'+ ":" +'"'+ (de_data)+'"'+"}"
                print login_token_str
                login_token_json=eval(login_token_str)
                #从数据库中获取短信验证码并进行格式转换
                msg=getmsg()
                msg_str="{"+'"'+"send_code"+'"'+ ":" +'"'+ msg +'"'+"}"
                #msg_json= json.loads(msg_str)
                msg_json=eval(msg_str)
                print type(msg_json)
                if  param:
                    print "is none"
                    param1=eval(param)
                    data2=dict(login_token_json.items()+param1.items()+msg_json.items())
                else:
                    print "no none"
                    data2=login_token_json
                headers = {"Content-Type": "application/x-www-form-urlencoded"}
                results2 = requests.post(url=url,data=data2,headers=headers).text
                print results2
                if len(results2)>32767:
                    responses.append("ERROR:LONG  LENGTH")
                else:
...
def copy_excel(file_path, res_flags, request_urls, responses):
    ...
    new_book.save(u'%s_验证码.xls' % time.strftime('%Y%m%d%H%M%S'))
